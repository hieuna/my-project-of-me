...
;
window.navbar={};
(function(){var byID=function(elemID){return document.getElementById(elemID);
},nullFn=function(){};
var agent=(new function(){function contains(){var args=Array.prototype.slice.call(arguments,0);
...
jQuery.ajax({url:_navbar.yourAccountPrimer,cache:false,type:"GET"});
};
})());
}function NavButton(id){this._jqId="#"+id;
var button=this;
byID(id).className=byID(id).className.replace("nav-menu-inactive","nav-menu-active");
this.onShow=function(){button._toggleClass(true,"nav-button-outer-open");
};
...
}var SKIN=function(jqObject,xOffset){var callback=function(){var navWidth=$id("nav-bar-outer").width();
...
}var mt=$.AmazonPopover.mouseTracker,ub=$.AmazonPopover.updateBacking,sbd_config=_navbar._sbd_config,subcatInited,promosInited,deferredResizeSubcat,catWidth,mtRegions=[],activeCat,wasSuperCat,subcatRect,delayedChange,priorCursor,exposeSBDData;
var id="nav-shop-all-button",button=new NavButton(id),jQButton=$id(id);
...
$browse_flyout.css({height:$cats.outerHeight()+"px",width:catWidth+"px"});
$subcats_wrap.css({display:"block"});
subcatInited=true;
}else{$browse_flyout.css({height:$cats.outerHeight()+"px"});
...
var doCatChange=function(cat,args){var animateSubcat=!activeCat,resizeSubcat=false,$subcat=$id("nav_subcats_"+cat),$cat=$id("nav_cat_"+cat);
...
}if(!_navbar.responsivegw||agent.touch){exposeSBDData={"ok":function(){return false;
},"deBorder":nullFn,"initDeferredShow":nullFn,"deferredShow":nullFn};
return exposeSBDData;
}var $anchor=$id("nav_exposed_anchor"),skin=SKIN(jQButton,0),$skin=$(typeof(skin)=="function"?skin():skin),$wrap=$('<div id="nav_exposed_skin"></div>').css({"top":-8,"left":jQButton.offset().left-(isIE6?27:30)}).append($skin).appendTo($anchor),$old_parent=$id("nav_browse_flyout"),$parent=$('<div id="nav_exposed_cats"></div>').appendTo($(".ap_content",$wrap)),$fade=$id("nav-shop-all-button").clone().attr({"href":"javascript:void(0)","id":""}).addClass("nav-shop-all-button nav-button-outer-open").appendTo("#nav-bar-inner").add($wrap),$roots=$id("navbar").add($anchor),$cats=$id("nav_cats_wrap"),exButton=new NavButton("nav_exposed_skin"),isExposed,firstCall=true,lastState,deferShow,deferHide;
...
var exTriggerParams={localContent:"#nav_browse_flyout",location:"top",locationAlign:"left",locationOffset:[30,($.browser.msie?33:32)],skin:SKIN($parent,0),hoverShowDelay:0,onShow:function(){if(!deferHide){deferHide=nullFn;
...
_navpreload._protectExposeSBD(function(expose){lastState=expose;
if(expose===isExposed){return;
}if(expose){var doShow=function(){if(lastState===false){return;
...
firstCall=false;
}else{button.registerTrigger(triggerParams);
}isExposed=false;
};
...
}var id="nav-your-account",button=new NavButton(id),jQButton=$id(id);
var onShow=function(){button.onShow();
areaMapper.disable("#nav_your_account_flyout");
window._navbar.loadDynamicMenusConditionally();
};
var onHide=function(){areaMapper.enable();
button.onHide();
};
button.registerTrigger({localContent:"#nav_your_account_flyout",locationAlign:"left",locationOffset:[isIE6?3:0,0],skin:SKIN(jQButton,0),onShow:onShow,onHide:onHide,followLink:window._navbar.yourAccountClickable&&!agent.touch});
};
var initCart=function(){if(!$id("nav-cart").length||!$id("nav_cart_flyout").length){return;
}var id="nav-cart",button=new NavButton(id),jQButton=$id(id);
var onShow=function(){button.onShow();
...
}var id="nav-wishlist",button=new NavButton(id),jQButton=$id(id),flyout="#nav_wishlist_flyout",ul=".nav_pop_ul";
var onShow=function(){button.onShow();
...
var onHide=function(){$(flyout).css("width","").find(ul).css("width","").removeClass("nav_pop_ul_wrap");
areaMapper.enable();
button.onHide();
};
...
}var cartCounts=$("#nav-cart-count, #nav-cart-count-flyout");
...
}}}}cartCounts.removeClass("nav-cart-0 nav-cart-10 nav-cart-20 nav-cart-100").addClass(cartFullClass).html(newCount).siblings(".nav-cart-plural")[newCount==1?"hide":"show"]();
});
...
}function updateDynamicMenu(status,dataList,parentElem,templateID,contentElem){if(status){if(dataList.length==0){parentElem.addClass("nav-empty");
...
}if(window._navbar.dynamicMenuUrl){$(attachTriggers);
}})();
...
})(),state={isHover:false,isFocus:false,searchFocus:false,searchHover:false},previousVal=null,leftward=$("form.nav-left-search-in").size()>0;
if($.browser.msie){if(parseFloat($.browser.version)<7){return;
}facade.addClass("ie");
...
}function updateFacade(force,useDefault){var selected=getSelected(),selectedVal=selected.val();
if(force===true||selectedVal!=previousVal){var display=((previousVal&&useDefault!==true)||(selectedVal!=facadePrefilled))?selected.html():facadeContent.innerHTML;
previousVal=selectedVal;
if(force===true||facadeContent.innerHTML!=display){facadeContent.innerHTML=display;
if(!leftward){searchBoxParent.css({"padding-right":facade.width()});
}else{facade.css({"width":"auto"});
searchBoxParent.css({"padding-left":facade.width()});
redrawSearchBox();
}tweakDropdownWidth();
}}updateActiveState();
}function focusSearchBox(){if(leftward&&iss){state["isFocus"]=false;
iss.focus();
...
updateFacade();
dropdown.change(updateFacade);
$(window).resize(redrawSearchBox);
$(function(){if($(facadeContent).outerWidth()-facade.outerWidth()>=4){updateFacade(true);
}else{if(!leftward){updateFacade(true,true);
}}tweakDropdownWidth();
dropdown.keyup(keypressHandler).focus(buildCallback("isFocus",true)).blur(buildCallback("isFocus",false)).hover(buildCallback("isHover",true),buildCallback("isHover",false)).navOnOptionClick(function(){focusSearchBox();
updateFacade();
});
if(document.activeElement==dropdown.get(0)){state["isFocus"]=true;
updateActiveState();
}dropdown.css({"top":Math.max(0,(facade.outerHeight()-dropdown.outerHeight())/2)});
});
amznJQ.available("search-js-autocomplete",function(){iss.keydown(keypressHandler);
if(leftward){iss.onFocus(buildCallback("searchFocus",true));
...
window.navbar.exposeSBD=function(expose){amznJQ.available("navbarJSInteraction",function(){if(_navbar._initExposeSBD().ok()){_navpreload._protectExposeSBD(expose);
}});
};
...
}}opt.form&&opt.form.submit(function(){var formElement=opt.form;
var formAction=formElement.attr("action");
if(!noissre.test(formAction)){return;
}var i=0;
var currentKeyword=keyword();
while(i<suggestionList.length){var entry=suggestionList[i].keyword;
if(normalize(entry)==normalize(currentKeyword)){formAction=formAction.replace(refre,"ref=nb_sb_noss_1");
...
while(i+j<maxSuggestions&&i<n){sg={keyword:crtSuggestions[i],sc:isSpellCorrection(extraData,i),conf:extraData[i].conf};
