	  ...
	  */	   
	  function get_first_page_nav($user_link="",$link_para="")
      {
        ...
        if(!$this->is_first_page()&& $this->show_first_last)
        {
        	...
        	return ' <a href="'.$this->page_name.'?page=0'.$this->qry_str.'" '.$link_para.' class="Orange_link">'.$user_link.'</a> ';
		}
		elseif($this->show_first_last && $this->show_disabled_links)
        {
			$user_link="<span class='Text_orange'>$user_link</span>";
			return $user_link;
		}	
      }
      function get_last_page_nav($user_link="",$link_para="")
      {
        ...
        {    
        	...
        	return ' <a href="'.$this->page_name.'?page='.($this->total_page-1).$this->qry_str.'" '.$link_para.' class="Orange_link">'.$user_link.'</a> ';
		}
		elseif($this->show_first_last && $this->show_disabled_links)
        {    
			$user_link="<span class='Text_orange'>$user_link</span>";
      ...
      function get_next_page_nav($user_link="",$link_para="")
      {
		if($this->total_page<=1)
			return;
	  	if(trim($user_link)=="")
			$user_link='<img src="asset/images/arr_right.gif" width="6" height="10" border="0">';
        if(!$this->is_last_page()&& $this->show_prev_next)
		{
            //echo ' <a href="'.$this->page_name.'?page='.($this->page+1).$this->qry_str.'" '.$link_para.'>'.$user_link.'</a> ';
        	return ' <a href="'.$this->page_name.'?page='.($this->page+1).$this->qry_str.'" '.$link_para.' class="content">'.$user_link.'</a> ';
		}
      ...
      function get_prev_page_nav($user_link="",$link_para="",$link_get="")
      {
		if($this->total_page<=1)
			return;
	  	if(trim($user_link)=="")
			$user_link='<img src="asset/images/arr_left.gif" width="6" height="10" border="0">';
        if(!$this->is_first_page()&& $this->show_prev_next)
        {
		    //echo ' <a href="'.$this->page_name.'?page='.($this->page-1).$this->qry_str.'" '.$link_para.'>'.$user_link.'</a> ';
			return ' <a href="'.$this->page_name.'?page='.($this->page-1).$this->qry_str.$link_get.'" '.$link_para.' class="content">'.$user_link.'</a> ';
		}	
        elseif($this->show_prev_next && $this->show_disabled_links)
		{
            //echo $user_link;
      ...
      function get_scroll_prev_page_nav($user_link="",$link_para="")
      {
        ...
        {    
			...
			return ' <a href="'.$this->page_name.'?page='.($this->page-$this->scroll_page).$this->qry_str.'" '.$link_para.' class="link_page1">'.$user_link.'</a> ';
		}	
      ...
      function get_number_page_nav($link_para="")
      {
        ...
        for(;$i<$j+$scroll_page && $i<$this->total_records;$i++)
        {
			 ...
			 {   
				...
				$str.=' <a href="'.$this->page_name.'?page='.$i.$this->qry_str.'" '.$link_para.' class="link_page1">'.($i+1).'</a> ';
			 }     
		}
		return $str;
      }

      function get_page_nav($link_get="")
      {
		...
		{	
         ...
         $str.=$this->get_prev_page_nav("",$this->link_para,$this->link_get)." ";
