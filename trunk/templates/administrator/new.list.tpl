      ...
      <tr>
        <td colspan="2"><form action="" method="post" name="frmList" id="frmList">
          <table width="100%"  border="0" align="center" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC" style="BORDER-COLLAPSE: collapse">
            ...
            <tr  class="stlContent">
              <td align="center">
			  <input name="chkItem" type="checkbox" id="chkItem" onClick="SelectItem(this)" value="{$r[i].news_id}"></td>
              <td><a href="new.edit.php?news_id={$r[i].news_id}" class="mainLink">{$r[i].news_title}</a></td> 
			  <td align="center" class="txt_normal">{$r[i].news_cat}</td>
			  <td align="center" class="txt_normal">{$r[i].news_sums|truncate:50}</td>              <td align="center" class="txt_normal">{$r[i].news_date}</td>
			  <td align="center" class="txt_normal">{if ($r[i].news_status==1)}
			  Hiển thị
				{else}
			  Khóa			
			  {/if}</td>              <td align="center"class="txt_normal">{if ($r[i].news_hot==1)}
			  Tin nóng
				{else}
			  Tin không nóng			
			  {/if}</td>
			...
			{else}
              <tr bgcolor="#F5F5F5"  >
              <td align="center"><input name="chkItem" type="checkbox" id="chkItem" onClick="SelectItem(this)" value="{$r[i].news_id}"></td>
              <td><a href="new.edit.php?news_id={$r[i].news_id}" class="mainLink">{$r[i].news_title}</a></td>  
			   <td align="center" class="txt_normal">{$r[i].news_cat}</td>
			  <td align="center" class="txt_normal">{$r[i].news_sums|truncate:50}</td>              <td align="center" class="txt_normal">{$r[i].news_date}</td>
			  <td align="center" class="txt_normal">			  
			  {if ($r[i].news_status==1)}
			  Hiển thị
				{else}
			  Khóa			
			  {/if}
			  
			  </td>              <td align="center"class="txt_normal">{if ($r[i].news_hot==1)}
			  Tin nóng
				{else}
			  Tin không nóng			
			  {/if}</td>
