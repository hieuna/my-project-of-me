...
class crawl_afamily_vn extends gdnCrawler {	
	function __construct()
	{
		global $db;
		set_time_limit(0);
		$this->web_id 		= 1;
		$this->web_linkRoot = 'http://afamily.vn';
		return ;
	}
	...
	function getPages()
	{
		...
		foreach ($this->web_categories as $category){
			...
			$this->web_pagelinks[ $category['id'] ] = $category['link']."/page-#PAGENUMBER#";
		}
		return ;
	}
	...
	function getAllCrawled()
	{
		...
		foreach ($this->web_categories as $category){
			...
			while ($row = $db->database_fetch_assoc($results)){
				...
				if (strpos($row['source_introtext'],'(aFamily)- ')!==false){
					$row['source_introtext'] = str_replace('(aFamily)- ','',$row['source_introtext']);
					$row['source_fulltext'] .= '<div style="font-weight: bold" align="right">Theo Afamily</div>';
				}
	...
	function _getLinksInPage($pageLink, $page, $catID)
	{
		...
		$a_find = 'div[id=BoxLevel2_Content] span[class=link_contain_tit] a';
	...
	function _getContent($contentLink)
	{
	    ...
	    $item['time']     	= $this->_convertTime(trim($article->find('span[id=ctl00_ContentPlaceHolder1_ctl00_lblNewsDate]', 0)->plaintext));
	    $item['introtext']  = trim($article->find('span[id=ctl00_ContentPlaceHolder1_ctl00_lblNews_InitCont]', 0)->plaintext);
	    $item['fulltext'] 	= trim($article->find('div[id=ctl00_ContentPlaceHolder1_ctl00_divNewsContent]',0)->innertext);
	...
	function _filterImgs(& $fullcontent, $contentID, $img_path, $title)
	{
		...
		foreach ($html->find('img') as $img){
			...
			if (strpos($img->src, 'channelvn.net')===false){
	...
	function _convertTime($timeString)
	{
		...
		$timeStamp = mktime($temp3[0], $temp3[1], $temp3[2],$temp2[1],$temp2[0],$temp2[2]);
