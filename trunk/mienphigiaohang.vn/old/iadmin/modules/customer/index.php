...
if($action=='add')
{
	...
	if($_POST && $_POST['title']!="")
	{
	 ...
	 $title=$_POST['title'];
	  ...
	  mysql_query("INSERT INTO customer (title) values ('{$title}')") or die ('Khong the insert');
	  $mess=alert_success($config['domain'],"Thêm khách hàng thành công!");
	}
...
elseif($action=='edit')
{
	...
	if($_POST && $_POST['title']!=="")
	{
		 ...
		 mysql_query("UPDATE customer SET title='{$title}' where id='".$id."'") ;
...
else
{
	...
	if($_POST['submit'])
	{
		if($_POST['Selected'])
		{
			foreach($_POST['Selected'] as $udel)
			{
				...
				mysql_query("DELETE FROM customer WHERE id='$udel'");
	...
	$row=10;
	$div=7;
	$num_value=mysql_num_rows(mysql_query("SELECT id FROM customer ".$where));
	
	///////////////////////////PHAN TRANG
	$p = (isset($_GET['p'])) ? $_GET['p'] : "0";  				
	$start = 	$p*$row;
	$limit=		"limit ".$start.",".$row;
	$url_page="do=customer&id_module=".$id_module.$psearch;
	...
	$listques=list_customer($where,$limit);
	$smarty->assign('info1',$listques);
