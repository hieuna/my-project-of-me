...
if($action=='addnew')
{
	...
	if($_POST)
	{
	 ...
	 if($picture!="")
	 {
	  ...
	  mysql_query("INSERT INTO post (parent,picture,hide,module,date) values('{$parent}','{$picture}','{$hide}','{$id_module}','{$date}')") or die ('Khong the insert');
	   ...
	   foreach ($listlang as $value)
		{
			...
			{
				mysql_query("INSERT INTO content_translate(id_category, title, id_lang, quote,tag,body,type_parent) values('".$id_category."','".$_REQUEST['title_'.$value['code']]."','".$value['id']."','".$_REQUEST['quote_'.$value['code']]."','".$_REQUEST['tag_'.$value['code']]."','".$_REQUEST['body_'.$value['code']]."','post')");
			}
		}
		
	  $mess=alert_success($config['domain'],"Thêm bản tin thành công!");
	}
	...
	$smarty->assign("module_name",$view_path."/addnew.html");//////////////TRUY XUAT GIAO DIEN CUA MODULES
}
	...
	if($_POST)
	{
		   ...
		   foreach ($listlang as $value)
		{
			if($_REQUEST['title_'.$value['code']]!="")
			{
				...
				{
					mysql_query("INSERT INTO content_translate(id_category, title, id_lang, quote,tag,body,type_parent) values('".$id."','".$_REQUEST['title_'.$value['code']]."','".$value['id']."','".$_REQUEST['quote_'.$value['code']]."','".$_REQUEST['tag_'.$value['code']]."','".$_REQUEST['body_'.$value['code']]."','post')");
				}
				else
				{
...
elseif($_REQUEST['action']=='delnew')
{
	$id=$_REQUEST['id'];
	$r=mysql_fetch_array(mysql_query("SELECT picture FROM new where id='".$id."'"));
	$pic_name=$r['picture'];
	mysql_query("DELETE FROM post where id='".$id."'");
	mysql_query("DELETE FROM content_translate where id_category='".$id."' and type_parent='post'");
...
else
{
	...
	if($_POST['submit'])
	{
		if($_POST['Selected'])
		{
			foreach($_POST['Selected'] as $udel)
			{
				...
				mysql_query("DELETE FROM content_translate where id_category='".$udel."' and type_parent='post'");
				if(is_file("../templates/pictures/news/".$pic_name))
				{
					unlink('../templates/pictures/news/'.$pic_name);
				}
	...
	$url_page="do=new&id_module=".$id_module.$psearch;
