...
if($action=='addcontent')
{
	...
	if($_POST)
	{
	 ...
	 if($picture!="")
	 {
	  ...
	  mysql_query("INSERT INTO post (parent,hide,module) values('{$parent}','{$hide}','{$id_module}')") or die ('Khong the insert');
	   ...
	   foreach ($listlang as $value)
		{
			...
			{
				mysql_query("INSERT INTO content_translate(id_category, title, id_lang, body,type_parent) values('".$id_category."','".$_REQUEST['title_'.$value['code']]."','".$value['id']."','".$_REQUEST['body_'.$value['code']]."','content')");
			}
		}
		
	  $mess=alert_success($config['domain'],"Thêm nội dung trang thành công!");
	}
	...
	$smarty->assign("module_name",$view_path."/addcontent.html");//////////////TRUY XUAT GIAO DIEN CUA MODULES
}
...
elseif($action=='editcontent')
{
	...
	if($_POST)
	{
		...
		if($picture!="")
		 ...
		 mysql_query("UPDATE post SET  parent='{$parent}',  hide='{$hide}' where id='".$id."'") ;
		   ...
		   foreach ($listlang as $value)
		{
			if($_REQUEST['title_'.$value['code']]!="")
			{
				...
				{
					mysql_query("INSERT INTO content_translate(id_category, title, id_lang, body,type_parent) values('".$id."','".$_REQUEST['title_'.$value['code']]."','".$value['id']."','".$_REQUEST['body_'.$value['code']]."','content')");
				}
				else
				{
		...
		$mess=alert_success($config['domain'],"Cập nội dung trang thành công!");
    	$smarty->assign('mess',$mess);
	...
	$smarty->assign("module_name",$view_path."/editcontent.html");//////////////TRUY XUAT GIAO DIEN CUA MODULES
}


###################################DELETE ######################################

elseif($_REQUEST['action']=='delcontent')
{
	$id=$_REQUEST['id'];
	$r=mysql_fetch_array(mysql_query("SELECT picture FROM post where id='".$id."'"));
	$pic_name=$r['picture'];
	mysql_query("DELETE FROM post where id='".$id."'");
	mysql_query("DELETE FROM content_translate where id_category='".$id."' and type_parent='content'");
...
else
{
	...
	if($_POST['submit'])
	{
		if($_POST['Selected'])
		{
			foreach($_POST['Selected'] as $udel)
			{
				...
				mysql_query("DELETE FROM content_translate where id_category='".$udel."' and type_parent='content'");
				if(is_file("../templates/pictures/news/".$pic_name))
				{
					unlink('../templates/pictures/news/'.$pic_name);
				}
	...
	$url_page="do=content&id_module=".$id_module.$psearch;
	...
	$smarty->assign("module_name",$view_path."/listcontent.html");//////////////TRUY XUAT GIAO DIEN CUA MODULES
}
?>
