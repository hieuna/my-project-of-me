...
if($action=='addgal')
{
	...
	// $picture=$_FILES['picture']['name'];
	 $category=$_POST['category'];
	  ...
	  mysql_query("INSERT INTO post (url,hide,module,parent) 	values('{$url}','{$hide}','{$id_module}','{$category}')") or die ('Khong the insert');
	   ...
	   foreach ($listlang as $value)
		{
			 ...
			 if($_FILES['picture_slide_'.$value['code']]['name']!="")
	 	{
	 	if(check_extend_img($_FILES['picture_slide_'.$value['code']]['name'])==false)
	 	{
	 	...
	 	{
			$str=time()."_".$_FILES['picture_slide_'.$value['code']]['name'];
			$picture=$str;
			move_uploaded_file($_FILES['picture_slide_'.$value['code']]['tmp_name'],"../templates/pictures/gallarys/".$picture) or die('Khong the upload hinh');
	 	}
	 }
			...
			{
				mysql_query("INSERT INTO content_translate(id_category, title, id_lang, type_parent,picture_slide) values('".$id_category."','".$_REQUEST['title_'.$value['code']]."','".$value['id']."','galleries','".$picture."')");
			}
		}
		
	  $mess=alert_success($config['domain'],"Thêm galleries thành công!");
	}
	...
	$smarty->assign("module_name",$view_path."/addgal.html");//////////////TRUY XUAT GIAO DIEN CUA MODULES
}

###################################EDIT ######################################

elseif($action=='editgal')
{
	$id=$_REQUEST['id'];
	$id_module=$_REQUEST['id_module'];
	...
	if($_POST)
	{
		...
		$category=$_POST['category'];
		 ...
		 mysql_query("UPDATE post SET  url='{$url}', hide='{$hide}', parent='{$category}' where id='".$id."'") ;
		   ...
		   foreach ($listlang as $value)
		{
			...
			if($_FILES['picture_slide_'.$value['code']]['name']!="")
	 	{
            ...
            <div class="text">
	 		...
	 		{
				if($_REQUEST['oldpic_'.$value['code']]!="")
				{
					if(is_file("../templates/pictures/gallarys/".$_REQUEST['oldpic_'.$value['code']])){ unlink("../templates/pictures/gallarys/".$_REQUEST['oldpic_'.$value['code']]);}
				}
				
				$str=time()."_".$_FILES['picture_slide_'.$value['code']]['name'];
				$picture=$str;
				move_uploaded_file($_FILES['picture_slide_'.$value['code']]['tmp_name'],"../templates/pictures/gallarys/".$picture) or die('Khong the upload hinh');
				mysql_query("UPDATE content_translate SET picture_slide='".$picture."' where id_lang='".$value['id']."' and id_category='".$id."' and type_parent='galleries'");
	 		}
		 }
			...
			if($_REQUEST['title_'.$value['code']]!="")
			{
				...
				{
					mysql_query("INSERT INTO content_translate(id_category, title, id_lang, type_parent) values('".$id."','".$_REQUEST['title_'.$value['code']]."','".$value['id']."','galleries')");
				}
				else
				{
		...
		$mess=alert_success($config['domain'],"Cập nhật gallary thành công!");
    	$smarty->assign('mess',$mess);
	...
	$smarty->assign("module_name",$view_path."/editgal.html");//////////////TRUY XUAT GIAO DIEN CUA MODULES
}


###################################DELETE ######################################

elseif($_REQUEST['action']=='delgal')
{
	$id=$_REQUEST['id'];
	
	
	
	$r=mysql_query("SELECT picture_slide FROM content_translate where id_category='".$id."' and type_parent='galleries'");
	while($pic_name=mysql_fetch_array($r)){
	if(is_file("../templates/pictures/gallarys/".$pic_name['picture_slide']))
	{
		unlink("../templates/pictures/gallarys/".$pic_name['picture_slide']);
	}
	
	}
	mysql_query("DELETE FROM post where id='".$id."'");
	mysql_query("DELETE FROM content_translate where id_category='".$id."' and type_parent='galleries'");
/*	$smarty->assign('id_module',$id_module);
...
elseif($_REQUEST['action']=="del_pic")
{
	...
	$r=mysql_fetch_array(mysql_query("SELECT picture_slide FROM content_translate where id_category='".$id."' and type_parent='galleries' and id_lang='".$id_lang."'"));
	$pic_name=$r['picture_slide'];
	mysql_query("UPDATE content_translate SET picture_slide='' where id_category='".$id."' and id_lang='".$id_lang."'");
...
else
{
	...
	if($_POST['submit'])
	{
		if($_POST['Selected'])
		{
			...
			{
				$r=mysql_query("SELECT picture_slide FROM content_translate where id_category='".$udel."' and type_parent='galleries'");
				while($pic_name=mysql_fetch_array($r)){
				...
				mysql_query("DELETE FROM content_translate where id_category='".$udel."' and type_parent='galleries'");
			}
	...
	$url_page="do=gallary&id_module=".$id_module.$psearch;
	...
	$smarty->assign("module_name",$view_path."/listgal.html");//////////////TRUY XUAT GIAO DIEN CUA MODULES
}
?>
