...
if($action=='addques')
{
	 ...
	 if($picture!="")
	 {
	  ...
	  mysql_query("INSERT INTO post (hide,module) values('{$hide}','{$id_module}')") or die ('Khong the insert');
	   ...
	   foreach ($listlang as $value)
		{
			if($_REQUEST['question_'.$value['code']]!="")
			{
				mysql_query("INSERT INTO content_translate(id_category, id_lang, question,answer,type_parent) values('".$id_category."','".$value['id']."','".$_REQUEST['question_'.$value['code']]."','".$_REQUEST['answer_'.$value['code']]."','question')");
			}
		}
		
	  $mess=alert_success($config['domain'],"Thêm hỏi đáp thành công!");
	}
	...
	$smarty->assign("module_name",$view_path."/addques.html");//////////////TRUY XUAT GIAO DIEN CUA MODULES
}
...
elseif($action=='editques')
{
	...
	if($_POST)
	{
		   ...
		   foreach ($listlang as $value)
		{
			if($_REQUEST['question_'.$value['code']]!="")
			{
				...
				{
					mysql_query("INSERT INTO content_translate(id_category,  id_lang, question,answer,type_parent) values('".$id."','".$value['id']."','".$_REQUEST['question_'.$value['code']]."','".$_REQUEST['answer_'.$value['code']]."','question')");
				}
				else
				{
					mysql_query("UPDATE content_translate SET 
														question='".$_REQUEST['question_'.$value['code']]."',
														answer='".$_REQUEST['answer_'.$value['code']]."'
		...
		$mess=alert_success($config['domain'],"Cập nhật hỏi đáp thành công!");
    	$smarty->assign('mess',$mess);
	...
	$smarty->assign("module_name",$view_path."/editques.html");//////////////TRUY XUAT GIAO DIEN CUA MODULES
}


###################################DELETE ######################################

elseif($_REQUEST['action']=='delques')
{
	$id=$_REQUEST['id'];
	$r=mysql_fetch_array(mysql_query("SELECT picture FROM new where id='".$id."'"));
	$pic_name=$r['picture'];
	mysql_query("DELETE FROM post where id='".$id."'");
	mysql_query("DELETE FROM content_translate where id_category='".$id."' and type_parent='question'");
...
else
{
	...
	if($_POST['submit'])
	{
		if($_POST['Selected'])
		{
			foreach($_POST['Selected'] as $udel)
			{
				...
				mysql_query("DELETE FROM content_translate where id_category='".$udel."' and type_parent='question'");
				if(is_file("../templates/pictures/news/".$pic_name))
				{
					unlink('../templates/pictures/news/'.$pic_name);
				}
	...
	if(isset($_REQUEST['searchvl']))
	{
		...
		$where=" where question like '%".$txtsearch."%' and module='".$id_module."' and id_lang=18";
	...
	$url_page="do=question&id_module=".$id_module.$psearch;
	...
	$smarty->assign("module_name",$view_path."/listques.html");//////////////TRUY XUAT GIAO DIEN CUA MODULES
}
?>
