...
function checkAccessModule($module_id){
	...
	if($row = mysql_fetch_array($db_module->result)){
...
function checkAddEdit($right="add"){
    ...
    $db_query = "SELECT * 
				 ...
				 adm_loginname='" . $userlogin . "' AND adm_password='" . $password . "' AND adm_active=1
				 AND mod_id = " . $module_id;
                 
	$db_getright = new db_query($db_query);
...
function checkRowUser($table,$field_id,$record_id,$returnurl,$field_admin_id="admin_id"){
	$strreturn ='';
    ...
    if($adm = mysql_fetch_array($db_useradmin->result)){
		if($adm["adm_isadmin"]==1){
            ...
            $db_query_record = "SELECT ". $field_admin_id ." FROM " . $table . " WHERE " . $field_id . " = " . $record_id;
			$db_record = new db_query($db_query_record);
            
			$row=mysql_fetch_array($db_record->result);
			if($row[$field_admin_id] == $_SESSION["user_id"]){
				$strreturn = '';
				unset($db_record);
			}else{
...
function createMenuModule($name, $path, $action, $file){
	...
	for($i=0; $i<count($arrAction); $i++){
		$menuModule	.= '<div class="bg_left_menu" onmouseover="this.className=\'bg_left_menu_hover\'" onmouseout="this.className=\'bg_left_menu\'" onclick="parent.workFrame.location=\'../' . $path . '/' . $arrFile[$i] . '\'">';
