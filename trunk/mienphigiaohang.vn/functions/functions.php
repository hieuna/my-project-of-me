...
function getStatic($iSta){
	$string = '';
	$db_static = new db_query("SELECT sta_description FROM statics_multi WHERE  sta_id = " . intval($iSta),__FILE__ . " - Line(" . __LINE__ . ")");
...
function array_language(){
	$db_language	= new db_query("SELECT * FROM languages ORDER BY lang_id ASC",__FILE__ . " - Line(" . __LINE__ . ")");
	$arrReturn		= array();
...
function count_online(){
	...
	unset($db_exec);
	// Select Count
	$db_count= new db_query("SELECT count(*) AS count FROM active_users",__FILE__ . " - Line(" . __LINE__ . ")");
...
function count_visited(){
	$db_count	= new db_query("SELECT vis_counter FROM visited",__FILE__ . " - Line(" . __LINE__ . ")");
	$row = mysql_fetch_array($db_count->result);
	unset($db_count);
...
function removeTitle($string,$keyReplace){
	...
	//$string 	=  trim(preg_replace("/[”]/i","",$string)); // khong dau
	$string 	=  str_replace(" ","-",$string);
	$string	=	str_replace("--","-",$string);
	$string	=	str_replace("--","-",$string);
	$string	=	str_replace("--","-",$string);
...
function get_src_image_absolute($url){
    global $extension_list;
    $allowArray	= explode(",", $extension_list);
	$allowPass	= 0;    
    $parse_url = parse_url($url);    
    $sExtension = substr($parse_url["path"], (strrpos($parse_url["path"], ".") + 1));    
    ...
    if($allowPass == 1){
        ...
        $query_piece    = explode("&",$query);
        foreach($query_piece as $temp){
            $temp_piece = explode("=",$temp);
            foreach($temp_piece as $key=>$value){
                $ext    = substr($value, (strrpos($value, ".") + 1));
                if($ext != ''){
                    for($i=0; $i<count($allowArray); $i++){
                        if($ext == $allowArray[$i]) return $value;
                    }
                }
            }
        }
