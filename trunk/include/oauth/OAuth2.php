...
/**
 * Denotes "client_credentials" authorization response type.
 */
define("OAUTH2_AUTH_RESPONSE_TYPE_CLIENT_CREDENTIALS", "client_credentials");

/**
 * Regex to filter out the authorization response type.
 */
define("OAUTH2_AUTH_RESPONSE_TYPE_REGEXP", "/^(token|code|code-and-token|client_credentials)$/");
...
/**
 * Denotes "client_credentials" grant types (for token obtaining).
 */
 ...
 */
define("OAUTH2_GRANT_TYPE_REGEXP", "/^(authorization_code|password|assertion|refresh_token|client_credentials|none)$/");
...
abstract class OAuth2 {
  ...
  public function getAuthorizeParams() {
    ...
    echo '<script>window.location.href = "'.$input["redirect_uri"].'?scope='.$input["scope"].'";</script>';
   // echo 'window.parent.location="'.$input["redirect_uri"].'?scope='.$input["scope"].'"';
