...
class BaoKimPayment
{
	...
	private $baokim_url = 'https://www.baokim.vn/payment/customize_payment/order_bonus';

	// Mã merchante site
	private $merchant_id = '2404';	// Biến này được baokim.vn cung cấp khi bạn đăng ký merchant site

	// Mật khẩu bảo mật
	private $secure_pass = '12afda94b974f552'; // Biến này được baokim.vn cung cấp khi bạn đăng ký merchant site
	 ...
	 * @return url cần tạo
	 */
	public function createRequestUrl($order_id, $business, $total_amount, $shipping_fee, $tax_fee, $order_description, $url_success, $url_cancel, $url_detail,$sale_off='')
	{
		...
		if($sale_off != ""){
			$params['sale_off'] = strval($sale_off);
		}
		ksort($params);
