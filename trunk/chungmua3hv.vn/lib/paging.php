				...
				if( $iNumberPage < 1){
					...
					if( $iType == 1){
						...
						if( $this -> iCurrentPage > 1) 
							$sPrePage = "<a href='". str_replace('{i}', $this -> iCurrentPage -1 , $this -> sPagingPath)."'>Prev</a>";
						...
						if( $this -> iCurrentPage < $iNumberPage )						
							$sNextPage .= "<a href='". str_replace('{i}', $this -> iCurrentPage + 1, $this -> sPagingPath)."'>Next</a>";
						...
						for ( $i = $iStart; $i< $iStart + $this -> iNumberPageShow; $i ++){
							...
							elseif( $this -> iCurrentPage == $i){
								$sPaging .= "<span style=\"color:red; font-weight:bold\">{$i}</span>&nbsp;";
							}else{
						...
						// get pre page
						if( $this -> iCurrentPage > 1){							
							$sPrePage = "<a href='". str_replace('{i}', $this -> iCurrentPage -1 , $this -> sPagingPath)."'>&laquo;</a>&nbsp;<a href='". str_replace('{i}', $this -> iCurrentPage -1 , $this -> sPagingPath)."'>Prev</a>";											
						}
						
						// get next
						if( $this -> iCurrentPage < $iNumberPage){
							$sNextPage .= "<a href='". str_replace('{i}', $this -> iCurrentPage + 1, $this -> sPagingPath)."'>Next</a>&nbsp;<a href='". str_replace('{i}', $this -> iCurrentPage + 1, $this -> sPagingPath)."'>&raquo;</a>";
						}
