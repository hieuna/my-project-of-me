...
class actionBack extends VS_Module_Base  
{
	...
	function listItem()
	{
	 ...
	 //	$oDatagrid->setTable($table);		
		...
		$where.=" and Content_Type='".$this->type."' ";
		if($_GET[$this->_prefix.'Name'])
			$where.=" and Content_Title like '%".addslashes($_GET[$this->_prefix.'Title'])."%' ";
			
		$table= $oDb->getAll("select *,(select Group_Name from tblgroup where Group_ID=".$this->_prefix."GroupID) as category from {$this->table} {$where} {$orderby}");
	...
	function buildForm ( $task, $arrData = array() )
	{
		...
		$form -> addElement('text', $this->_prefix.'Title',  "Tiêu đề", array('size' => 50, 'maxlength' => 255) );
