...
class noticeBack extends VS_Module_Base  
{
	...
	function listItem()
	{
		...
		//$oDatagrid->where(" ");
		echo '<input type="button" onclick="document.location.href=\'index.php?mod=admin&amod=content&atask=notice&task=export\';" value="Xuất ra Excel" />
		<input type="button" onclick="document.location.href=\'index.php?mod=admin&amod=content&atask=notice&task=import\';" value="Import từ Excel" />';
		//$oDatagrid->displayGrid();
...
<input type=\"file\" name=\"fileImport\" />( Chọn file chứa email từ máy tính ví dụ: <i>danh-sach.csv</i>)
...
<input type=\"submit\" value=\"Import file\" /></p>
		...
		if($_SERVER['REQUEST_METHOD']=='POST'){
			if($_FILES['fileImport']['tmp_name']){
				
				// kiem tra xem file co dung dinh dang ko ////
				$dot = explode(".",$_FILES['fileImport']['name']);
				$ext = $dot[count($dot)-1];
				if($ext == 'csv'){
				 $_CONTENT =  file_get_contents($_FILES['fileImport']['tmp_name']); 
				 $_ARRCONTENT = explode("\n",$_CONTENT);
			 
				 if($_ARRCONTENT)
					foreach($_ARRCONTENT as $email){
						if($email!=""){
							$check = $this->getOne("select User_ID from tbl_notice_deals where User_email='{$email}'");
							if(!$check){
	...
	function export()
	{		
		$email= $this->getCol("select User_email from tbl_notice_deals where User_Active='1' order by  User_email asc");
		if($email){
			$index=0; $_STRING="";
		//********* tao file csv ///////////////////******////
			foreach($email as $colum){
				...
				if($index % 500 == 0 and $index >= 500){
					$this->writeFileCsv("listEmail{$index}.csv",$_STRING); // ghi ra file moi file toi da 500 email
			...
			if(count($email) < 500){
					$this->writeFileCsv("listEmailFirst.csv",$_STRING); // ghi ra file moi file toi da 500 email
...
/////////*****************************************************************************////////		
			...
			while (($file = readdir($dh)) !== false) {
				if ($file != '.' && $file != '..')
				{
					echo "<A HREF=\"".SITE_URL."upload/email/$file\">$file</A><BR>\n";
				}
			}
...
///88*******************************************/888888888/*************///////////////
	...
	function writeFileCsv($filename,$content){
			$file_path = SITE_DIR."upload/email/".$filename;			
	...
	//	$form -> addElement('date', $this->_prefix.'CreateDate',  "Ngày", array('size' => 50, 'maxlength' => 255) );
		...
		$form -> addElement('checkbox', $this->_prefix.'Active', "Kích hoạt");
