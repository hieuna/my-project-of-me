...
class NewsBack extends VS_Module_Base  
{
	...
	function listItem()
	{
	 ...
	 //	$oDatagrid->setTable($table);		
		...
		$table= $oDb->getAll("select *,(select name from news_category where id = category_id) as category from {$this->table} {$where} {$orderby}");
	...
	function buildForm ( $task, $arrData = array() )
	{
		...
		if ( $arrData['photo']!= "" )
		{
			...
			$sImage = $this -> showFlash(GUIDE_URL.$this->imagePath.$arrData['photo'])."<br>
		...
		$form -> addElement('textarea', 'des',  "Miêu tả", array('cols' => 130, 'rows' => 5) );
		$form -> addElement('static', NULL,  "Nội dung", $comment);		
		
		$form -> addElement('checkbox', 'active', $this->get_config_vars('status'));
		...
		$form->addElement( 'hidden', 'oldPhoto', $arrData['photo'] );
		...
		if( $form -> validate())
		{
			...
			if($_FILES['photo']['name']){
				...
				if($uploadFile=='none' || $uploadFile == '') $aData['photo'] = "";
				else
					$aData['photo'] = $uploadFile;
	...
	function uploadPhoto ()
	{
		...
		$folder = GUIDE_DIR.$this->imagePath;
