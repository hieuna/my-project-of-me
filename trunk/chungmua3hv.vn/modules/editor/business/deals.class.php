...
class DealsBack extends VS_Module_Base  
{
	function __construct(){
		...
		$this->vsDb->setPrimaryKey("Deals_ID");
		$this->_prefix ="Deals_";	
		$this->imagePath ="upload/deals/";	
	...
	function listItem()
	{
	 ...
	 //	$oDatagrid->setTable($table);		
		...
		$arr_cols= array(					
			...
			array(
				"field" => $this->_prefix."ToDate",
				"display" => "Date",
				"style" =>" font-size:11px; width:90px; text-align:center;",
				"datatype"	=> "date",
				),
			array(
				"field" => $this->_prefix."Hot",
				"display" => "Hot",
				"style" =>" font-size:11px; width:90px; text-align:center;",
				"datatype"	=> "publish",
				)
		...
		$table= $oDb->getAll("select *,(select Business_Name from tbl_business where Business_ID= Deals_BusinessID) as business from {$this->table} {$where} {$orderby}");
	...
	//$category = $oDb->getAssoc("select Group_ID, Group_Name from tblgroup");
		$type= array("free"=>"Free","premium"=>"Premium");
