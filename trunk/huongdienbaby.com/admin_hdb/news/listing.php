...
$db_count = new db_query("SELECT Count(*) AS count
						  FROM news
						  INNER JOIN categories_multi ON (cat_id = new_category)
						  WHERE news.lang_id = " . $_SESSION["lang_id"] . $sqltype . $sqlCat);
...
$db_list = new db_query("SELECT *
						 ...
						 INNER JOIN categories_multi ON (cat_id = new_category)
						 WHERE news.lang_id = " . $_SESSION["lang_id"] . $sqltype . $sqlCat . "
						 ORDER BY new_date DESC,cat_id ASC,new_title ASC
             ...
             <td><input type="text" class="form" name="keyword" value="<?=$keyword?>" style="width:120px" /></td>
				 ...
				 <td>
				 	<select class="form" name="filter">
						...
						?>
							<option value="<?=$key?>" <? if($key==$filter) echo 'selected';?>><?=$value?></option>
						<?
						}
						?>
				 	</select>
		...
		<table border="1" bordercolor="#E5ECF9" cellpadding="3" cellspacing="2" width="100%" style="border-collapse:collapse">
			...
			<tr>
				...
				<td>
					...
					<? /*-----------------*/ ?>				</td>
				<td align="center"><textarea name="new_teaser" id="new_teaser" cols="60" rows="7" class="form"><?=preg_replace('|'. quotemeta('<') .'(.*)' . quotemeta('>') . '|U','',$row["new_teaser"])?></textarea></td>
				<td>
