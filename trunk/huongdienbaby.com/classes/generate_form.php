...
class generate_form{
	...
	*/
	function add($data_field, $data_value, $data_type, $data_store, $data_default_value, $data_require=0, $data_error_message="", $data_unique=0, $data_error_message2="",$type_form = 0){
		//neu add 2 lan
		if(array_search($data_field,$this->array_data_field)) $this->strErrorFeld .= "&bull; add 2 lần trường <strong>" . $data_field . "</strong> <br>";
	...
	*/
	function addTable($table_name){
		...
		$arrayField["lang_id"]	= count($this->array_data_field);
		...
		foreach($arrayField as $key=>$value){
			if(!array_key_exists($key,$arrayFieldIntable)) $this->strErrorFeld .= "&bull; không tồn tại trường <strong>" . $key . "</strong> trong bảng <strong>" . $this->table_name . "</strong> <br>";
		}
		unset($db_field);
		unset($row);
		unset($arrayField);
