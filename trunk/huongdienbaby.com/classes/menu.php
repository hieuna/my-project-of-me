...
class menu
{
	...
	*/
	...
	function getAllParentList($table_name,$id_field,$name_field,$parent_id_field,$id,$seperate_str,$class_link="link_title",$modlue="product",$type_list=0,$lang_path,$con_extenstion,$con_mod_rewrite){
		...
		while (!$finish){
			$db_getparent = new db_query ("SELECT " . $parent_id_field . "," . $name_field . " " . (($modlue=='') ? ',cat_type' : '') . 
													" FROM " . $table_name . " " .
													" WHERE " . $id_field . "=" . $current_id);
			if ($row=mysql_fetch_array($db_getparent->result)){
				$count_var++;
				$link = createLink("type",array('module'=>(($modlue=='') ? $row["cat_type"] : $modlue),"title"=>$row[$name_field],"iCat"=>$current_id),$lang_path,$con_extenstion,$con_mod_rewrite);
				$link_cat = createLink("type",array('module'=>(($modlue=='') ? $row["cat_type"] : $modlue),"title"=>$row[$name_field],"iCat"=>$iCat),$lang_path,$con_extenstion,$con_mod_rewrite);
				//ghep vao chuoi str
				if ($parent_list==""){ 
					if($type_list==1){
						
						$parent_list = "<a href=\"" . $link . "\" class=\"" . $class_link . "\">" . $row[$name_field] . "</a>";
					}else{
						$parent_list = "" . $row[$name_field] . "";
					}
				}else{
					if($type_list==0){
						$parent_list ="<a href=\"" . $link . "\" class=\"" . $class_link . "\">" . $row[$name_field] . "</a>" . $seperate_str . "<b>" . $parent_list . "</b>";
					}else{
						$parent_list ="<a href=\"" . $link . "\" class=\"" . $class_link . "\">" . $row[$name_field] . "</a>" . $seperate_str . "<a href=\"" . $link_cat . "\" class=\"" . $class_link . "\">" . $parent_list . "</a>";
					}
				}
	...
	function getArray($table_name,$id_field,$parent_id_field,$where_clause = "1"){
		$db_getparent = new db_query ("SELECT " . $parent_id_field . ',' . $id_field .
												" FROM " . $table_name . " " .
												" WHERE " . $where_clause . ' AND ' . $parent_id_field . " <> 0");
	...
	*/
	function getComboxJava($table_name,$id_field,$name_field,$parent_id_field,$where_clause = "1"){
		...
		$i=-1;
		$cat_parent 	= -1;
		$return 		= '<script language="javascript"> /* dinhtoan1905@gmail.com  */ var regiondb = new Array();';
		while($row=mysql_fetch_array($db_getparent->result)){
			if($row[$parent_id_field]==0){
				...
				if($cat_parent != $row[$parent_id_field]){
					...
					$return 	.= 'regiondb[' . $row[$parent_id_field] . '] = new Array();';
				}
				$i++;
				$return .= 'regiondb[' . $row[$parent_id_field] . '][' . $i . '] = new Array(' . $row[$id_field] . ',"' . htmlspecialchars($row[$name_field]) . '");';
			}
		}
