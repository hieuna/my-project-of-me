window.addEvent("load",function(){
	$$(".gkIsWrapper-gk_memovie").each(function(el){
		...
		if(wrapper.getElement('.gkIsPrev')) {
			...
			if(wrapper.getElement('.gkIsPrev').hasClass('anim')) {
			...
			if(loadedImages){
				...
				if($G['autoanim']){
					$G['actual_animation'] = (function(){
						gk_is_gk_memovie_anim(wrapper, contents, slides, $G['actual_slide']+1, $G);
...
function gk_is_gk_memovie_anim(wrapper, contents, slides, which, $G){
