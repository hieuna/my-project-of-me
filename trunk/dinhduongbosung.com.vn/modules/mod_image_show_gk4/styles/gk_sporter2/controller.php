...
class GKIS_gk_sporter2_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function checkImages() {
		// basic images params		
		$img_width = $this->config['config']->gk_sporter2->gk_sporter2_image_width;
		$img_height = $this->config['config']->gk_sporter2->gk_sporter2_image_height;
		$thumb_width = $this->config['config']->gk_sporter2->gk_sporter2_thumb_width;
		$thumb_height = $this->config['config']->gk_sporter2->gk_sporter2_thumb_height;
		$img_bg = $this->config['config']->gk_sporter2->gk_sporter2_image_bg;
		$quality = $this->config['config']->gk_sporter2->gk_sporter2_quality;
		// check the slides
		foreach($this->config['image_show_data'] as $slide) {
			$stretch = ($slide->stretch == 'nostretch') ? false : true;
			GKIS_Sporter2_Image::createThumbnail($slide->image, $this->config, $img_width, $img_height, $img_bg, $stretch, $quality);
			GKIS_Sporter2_Image::createThumbnail($slide->image, $this->config, $thumb_width, $thumb_height, $img_bg, $stretch, $quality, 'thumb_');	
		}
	}
	// get the articles data
    function getArticleData() {
		...
		// get the data
		if(count($idsK2) > 0) {
			$this->articlesK2 = GKIS_gk_sporter2_Model::getDataK2($idsK2);
		}
		if(count($ids) > 0) {
			$this->articles = GKIS_gk_sporter2_Model::getData($ids);
		}
	}
	// generate view
	function generateView() {
		...
		$width = $this->config['config']->gk_sporter2->gk_sporter2_image_width;
		$height = $this->config['config']->gk_sporter2->gk_sporter2_image_height;
		// add script fragment
		$document->addScriptDeclaration('try {$Gavick;}catch(e){$Gavick = {};};$Gavick["gkIs-'.$this->config['module_id'].'"] = { "width": '.$width.', "height": '.$height.' };');
		// load view
