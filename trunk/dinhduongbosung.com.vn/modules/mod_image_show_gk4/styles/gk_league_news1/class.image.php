...
class GKIS_LeagueNews1_Image {
	...
	*/
	function translateName($name, $mod_id, $thumbnail = false) {
		$name = GKIS_LeagueNews1_Image::getRealPath($name);
		$start = strpos($name, DS.'images'.DS);
		$name = substr($name, $start+8);
		$ext = substr($name, -4);
		$name = substr($name, 0, -4);
		$name = str_replace(DS,'.',$name);
		$name .= $mod_id.($thumbnail ? '_small' : '').$ext;
		return $name;
	}
	...
	*/
	...
	// Creating thumbnails
	function createThumbnail($path, $config, $width, $height, $image_bg, $image_stretch, $quality, $thumbnail = false) {
		if(GKIS_LeagueNews1_Image::checkCache(GKIS_LeagueNews1_Image::translateName($path,$config['module_id'], $thumbnail), $config['last_modification'], $config['module_id'])){
			return TRUE;	
		}else{
			// importing classes
			...
			$cache_dir = JPATH_ROOT.DS.'modules'.DS.'mod_image_show_gk4'.DS.'cache'.DS;
			// file path
			$file = GKIS_LeagueNews1_Image::getRealPath($path);
			// filename
			$filename = GKIS_LeagueNews1_Image::translateName($path,$config['module_id'], $thumbnail);
