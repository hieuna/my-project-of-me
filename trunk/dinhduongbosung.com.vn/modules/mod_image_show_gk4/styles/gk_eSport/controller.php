...
class GKIS_gk_eSport_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function getArticleData() {
		...
		// get the data
		if(count($ids) > 0) {
			$this->articles = GKIS_gk_eSport_Model::getData($ids);
		}
		if(count($idsK2) > 0) {
			$this->articlesK2 = GKIS_gk_eSport_Model::getDataK2($idsK2);
		}
	}
	// generate view
	function generateView() {
		...
		// add script fragment
		$document->addScriptDeclaration('try {$Gavick;}catch(e){$Gavick = {};};$Gavick["gkIs-'.$this->config['module_id'].'"] = { "slide_interval": '.$this->config['config']->gk_eSport->gk_eSport_slide_interval.', "anim_speed": '.$this->config['config']->gk_eSport->gk_eSport_animation_speed.', "anim_interval": '.$this->config['config']->gk_eSport->gk_eSport_animation_interval.', "autoanim": '.$this->config['config']->gk_eSport->gk_eSport_autoanimation.', "row_amount": '.$this->config['config']->gk_eSport->gk_eSport_row_amount.', "lvl2_divider": '.$this->config['config']->gk_eSport->gk_eSport_lvl2_divider.', "max_anim": '.$this->config['config']->gk_eSport->gk_eSport_max_anim.', "slide_links": '.$this->config['config']->gk_eSport->gk_eSport_slide_links.' };');
		// generate necessary variables
		$width = $this->config['config']->gk_eSport->gk_eSport_image_w;
		$height = $this->config['config']->gk_eSport->gk_eSport_image_h;
		$thumb_width = $this->config['config']->gk_eSport->gk_eSport_thumb_w;
		$thumb_height = $this->config['config']->gk_eSport->gk_eSport_thumb_h;
		// load view
