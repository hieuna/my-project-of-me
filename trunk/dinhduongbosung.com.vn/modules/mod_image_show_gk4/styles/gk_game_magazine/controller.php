...
class GKIS_gk_game_magazine_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function getArticleData() {
		...
		// get the data
		if(count($idsK2) > 0) {
			$this->articlesK2 = GKIS_gk_game_magazine_Model::getDataK2($idsK2);
		}
		if(count($ids) > 0) {
			$this->articles = GKIS_gk_game_magazine_Model::getData($ids);
		}
	}
	...
	function generateView() {
		...
		$width = $this->config['config']->gk_game_magazine->gk_game_magazine_image_width;
		$height = $this->config['config']->gk_game_magazine->gk_game_magazine_image_height;
		// add script fragment
		$document->addScriptDeclaration('try {$Gavick;}catch(e){$Gavick = {};};$Gavick["gkIs-'.$this->config['module_id'].'"] = { "width": '.$width.', "height": '.$height.', "anim_speed": '.$this->config['config']->gk_game_magazine->gk_game_magazine_animation_speed.', "anim_interval": '.$this->config['config']->gk_game_magazine->gk_game_magazine_animation_interval.', "autoanim": '.$this->config['config']->gk_game_magazine->gk_game_magazine_autoanimation.', "anim_type": "'.$this->config['config']->gk_game_magazine->gk_game_magazine_animation_type.'", "anim_text_type": "'.$this->config['config']->gk_game_magazine->gk_game_magazine_animation_text_type.'", "slide_links": '.$this->config['config']->gk_game_magazine->gk_game_magazine_slide_links.' };');
		// load view
