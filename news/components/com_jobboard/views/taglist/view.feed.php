...
class JobboardViewList extends JView
{
      function display($tpl = null)  {
        ...
        $catid = intval($catid);
        $keywd = trim( JRequest::getVar('keysrch', '', 'get','string') );
        ...
        $query = 'SELECT use_location FROM #__jobboard_config WHERE id=1';
        $db->setQuery($query);
        $use_location = $db->loadResult();        

        // get the items to add to the feed
        $where = ($catid == 1 || $catid == 0)? '' : ' WHERE c.id ='.intval($catid);
        
        $tag_include = strlen($keywd);
        if($tag_include > 0 && $catid == 0)  {
        	$tag_requested = $this->checkTagRequest($keywd);
        	$where .= ($tag_requested <> '')? " WHERE j.job_tags LIKE '%{$tag_requested}%' " : '';
        }
        $query = 'SELECT
                      j.id
                      , j.post_date
