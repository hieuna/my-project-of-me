...
class JobboardModelTagList extends JModel
{
	...
	function _buildQuery()
	{
		...
		//$category_id = $app->getUserStateFromRequest('jb_taglist.catid','catid','');
		...
		$order = trim($app->getUserStateFromRequest('jb_taglist.order','order',''));
		switch ($order)
		{
			case 'date':
		...
		$sort = trim($app->getUserStateFromRequest('jb_taglist.sort','sort',''));
		switch ($sort)
		{
			case 'a':
				$orderby = $orderby.' ASC';
	...
	function getData()
	{
		...
		if(empty($this->data))
		{
			...
			$app->setUserState('jb_taglist.count',$this->count);
		}
		return $this->data;
	}
