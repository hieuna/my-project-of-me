...
class crawlUp2Server {
	...
	function __construct()
	{
		...
		$this->server_db_host = '';
		$this->server_db_username = '';
		$this->server_db_password = '';
		$this->server_db_name = '';
	...
	function getContents()
	{
		...
		if ($this->web_source_id==4) $nextMonth = mktime(0,0,0,1,1,2010);
		else $nextMonth = time()-2592000;
		
		// GET TOTAL
		$sql = "SELECT NULL FROM content WHERE title<>'' AND introtext<>'' AND `full_text`<>'' AND source_linkdie=0 AND up2server=0 ".$where;
		$total = $this->db->database_query($sql);
		$this->total_contents = $this->db->database_num_rows($total);
		
		if ($this->total_contents){
			$sql = "SELECT id, title, introtext, full_text, sectionid, catid, created, source_web_id, source_link FROM content WHERE title<>'' AND introtext<>'' AND `full_text`<>'' AND source_linkdie=0 AND up2server=0 ".$where." ORDER BY created ASC LIMIT ".$this->limit_upload;
