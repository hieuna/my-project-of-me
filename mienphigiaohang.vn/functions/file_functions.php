...
function delete_pic($table_name,$id_field,$id_field_value,$field_select,$ff_imagepath){
	...
	if($row=mysql_fetch_array($db_select->result)){
		...
		if(file_exists($ff_imagepath . "normal_" . $row[$field_select])) @unlink($ff_imagepath . "normal_" . $row[$field_select]);
