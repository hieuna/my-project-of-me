...
class generate_form{
	...
	*/
	function addTable($table_name){
		...
		$db_field = new db_query("SHOW FIELDS FROM " . $this->table_name ,__FILE__ . " - Line(" . __LINE__ . ")");
	...
	*/
	function generate_insert_SQL($multi=0){
		...
		if($multi==1){
			...
			$querystr	= $this->type_insert . " " . $this->table_name . $str_field . " VALUES " . $str_data;
			return $querystr;
		}
	...
	*/
	function checkTypeData($i){
		...
		switch($this->array_data_type[$i]){
			//String
			case 0:
				$strreturn = "if (checkblank(document.getElementById(\"" . $this->array_data_value[$i] . "\").value)) { alert('" . htmlspecialchars($this->array_data_error_message[$i]) . "');  document.getElementById(\"" . $this->array_data_value[$i] . "\").style.border='" . $this->border_form_not . "'; document.getElementById(\"" . $this->array_data_value[$i] . "\").focus(); return false;}else{ document.getElementById(\"" . $this->array_data_value[$i] . "\").style.border='" . $this->border_form_ok . "'; }";
				break;
			//Integer
			case 1:
				$strreturn = "if (checkblank(document.getElementById(\"" . $this->array_data_value[$i] . "\").value)) { alert('" . htmlspecialchars($this->array_data_error_message[$i]) . "');  document.getElementById(\"" . $this->array_data_value[$i] . "\").style.border='" . $this->border_form_not . "'; document.getElementById(\"" . $this->array_data_value[$i] . "\").focus(); return false;}else{ document.getElementById(\"" . $this->array_data_value[$i] . "\").style.border='" . $this->border_form_ok . "'; }";
				$strreturn .= "if (isNaN(document.getElementById(\"" . $this->array_data_value[$i] . "\").value)) { alert('" . htmlspecialchars($this->array_data_error_message[$i]) . "');  document.getElementById(\"" . $this->array_data_value[$i] . "\").style.border='" . $this->border_form_not . "'; document.getElementById(\"" . $this->array_data_value[$i] . "\").focus(); return false;}else{ document.getElementById(\"" . $this->array_data_value[$i] . "\").style.border='" . $this->border_form_ok . "'; }";
				break;
			//Email
			case 2:
				$strreturn = "if (!isemail(document.getElementById(\"" . $this->array_data_value[$i] . "\").value)) { alert('" . htmlspecialchars($this->array_data_error_message[$i]) . "');  document.getElementById(\"" . $this->array_data_value[$i] . "\").style.border='" . $this->border_form_not . "'; document.getElementById(\"" . $this->array_data_value[$i] . "\").focus(); return false;}else{ document.getElementById(\"" . $this->array_data_value[$i] . "\").style.border='" . $this->border_form_ok . "'; }";
				break;
			//Double
			case 3:
				$strreturn = "if (checkblank(document.getElementById(\"" . $this->array_data_value[$i] . "\").value)) { alert('" . htmlspecialchars($this->array_data_error_message[$i]) . "');  document.getElementById(\"" . $this->array_data_value[$i] . "\").style.border='" . $this->border_form_not . "'; document.getElementById(\"" . $this->array_data_value[$i] . "\").focus(); return false;}else{ document.getElementById(\"" . $this->array_data_value[$i] . "\").style.border='" . $this->border_form_ok . "'; }";
	...
	function checkTypeform($id,$type,$message){
		...
		switch($type){
			//String
			case 0:
				$strreturn = "if (checkblank(document.getElementById(\"" . $id . "\").value)) { alert('" . htmlspecialchars($message) . "');  document.getElementById(\"" . $id . "\").style.border='" . $this->border_form_not . "'; document.getElementById(\"" . $id . "\").focus(); return false;}else{ document.getElementById(\"" . $id . "\").style.border='" . $this->border_form_ok . "'; }";
				break;
			//Integer
			case 1:
				$strreturn = "if (checkblank(document.getElementById(\"" . $id . "\").value)) { alert('" . htmlspecialchars($message) . "');  document.getElementById(\"" . $id . "\").style.border='" . $this->border_form_not . "'; document.getElementById(\"" . $id . "\").focus(); return false;}else{ document.getElementById(\"" . $id . "\").style.border='" . $this->border_form_ok . "'; }";
				$strreturn .= "if (isNaN(document.getElementById(\"" . $id . "\").value)) { alert('" . htmlspecialchars($message) . "');  document.getElementById(\"" . $id . "\").style.border='" . $this->border_form_not . "'; document.getElementById(\"" . $id . "\").focus(); return false;}else{ document.getElementById(\"" . $id . "\").style.border='" . $this->border_form_ok . "'; }";
				break;
			//Email
			case 2:
				$strreturn = "if (!isemail(document.getElementById(\"" . $id . "\").value)) { alert('" . htmlspecialchars($message) . "');  document.getElementById(\"" . $id . "\").style.border='" . $this->border_form_not . "'; document.getElementById(\"" . $id . "\").focus(); return false;}else{ document.getElementById(\"" . $id . "\").style.border='" . $this->border_form_ok . "'; }";
				break;
			//Double
			case 3:
				$strreturn = "if (checkblank(document.getElementById(\"" . $id . "\").value)) { alert('" . htmlspecialchars($message) . "');  document.getElementById(\"" . $id . "\").style.border='" . $this->border_form_not . "'; document.getElementById(\"" . $id . "\").focus(); return false;}else{ document.getElementById(\"" . $id . "\").style.border='" . $this->border_form_ok . "'; }";
	...
	function checkdata($id_field="", $id_value=0){
		...
		for($i=0; $i<=$this->number_of_field; $i++){
			...
			if($this->array_data_unique[$i] == 1){
                ...
                $db_query_data_unique = "SELECT " . $this->array_data_field[$i] . "
                                                    FROM " . $this->table_name . "
                                                    WHERE " . $this->array_data_field[$i] . " = '" . $temp . "' " . $add_sql . " LIMIT 1";
                //return $db_query_data_unique;                                    
				$db_select = new db_query($db_query_data_unique, __FILE__ . " - Line(" . __LINE__ . ")" );
	...
	*/
	...
	function copyRecord($tableName,$field_id,$field_name,$record_id,$key='',$listFieldFile = '',$filePath = ''){
			...
			if($listFieldFile!=''){
				...
				$db_file 		= new db_query("SELECT " . $listFieldFile ." FROM " . $tableName . " WHERE " . $field_id . "=" . $record_id . " LIMIT 1",__FILE__ . " - Line(" . __LINE__ . ")");
			...
			$db_record		=	new db_execute("DROP TABLE IF EXISTS temp_" . $tableName,__FILE__ . " - Line(" . __LINE__ . ")");
			$db_record		=	new db_execute("CREATE table temp_" . $tableName . " SELECT * FROM " . $tableName . " WHERE " . $field_id . " = " . $record_id,__FILE__ . " - Line(" . __LINE__ . ")");
			if($key!=''){
				$db_record		=	new db_execute("update temp_" . $tableName . " SET " . $key . " = (select MAX(" . $key . ") FROM " . $tableName . ")+1 WHERE " . $field_id . " = " . $record_id);
			}
			$db_record		=	new db_query("SELECT MAX(" . $field_id . ") AS idmax FROM " . $tableName,__FILE__ . " - Line(" . __LINE__ . ")");
			$newid			=	0;
			...
			$db_record		=	new db_query("UPDATE temp_" . $tableName . " SET " . $field_id . "=" . $newid . $reFieldName . " WHERE " . $field_id . "=" . $record_id,__FILE__ . " - Line(" . __LINE__ . ")");
			$db_record		=	new db_execute("INSERT INTO " . $tableName . " SELECT * FROM temp_" . $tableName . " WHERE " . $field_id . " = " . $newid,__FILE__ . " - Line(" . __LINE__ . ")");
			$db_record		=	new db_execute("DROP TABLE IF EXISTS temp_" . $tableName,__FILE__ . " - Line(" . __LINE__ . ")");
			unset($db_record);
		return 	$newid;
	}
