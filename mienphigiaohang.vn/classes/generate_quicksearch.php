...
class generate_quicksearch{
	...
	function generate_quicksearch($keyword,$list_field,$keyword_reject='',$type_and_or=0){
		...
		$j=-1;
		preg_match_all('/\&quot;(.*?)\&quot;/is', $this->keyword,$quote);
		...
		for ($i=0;$i<count($this->array_keyword);$i++){
			...
			if ($this->array_keyword[$i][1]=="F"){
				$this->sql_keyword .= "MATCH(" . $this->list_field . ") AGAINST ('" . $this->array_keyword[$i][0] . "*' IN BOOLEAN MODE) AND ";
			}
			//Sử dụng LIKE
			else{
				$this->sql_keyword .= $this->list_field . " LIKE '%" . $this->array_keyword[$i][0] . "%' AND ";
			}
		}
		$this->sql_keyword .= "1 ";		
		...
		for ($i=0;$i<count($this->array_keyword_reject);$i++){
			$this->sql_keyword_reject .= $this->list_field . " NOT LIKE '%" . $this->array_keyword_reject[$i] . "%' AND ";
		}
...
/*
//chạy thử
$keyword='Cộng hòa + xã hội "chủ nghĩa" "việt nam"';
$keyword_reject='sdfdsfds';
$list_field='pro_search';
$mykey=new  generate_quicksearch($keyword,$keyword_reject,$list_field);
