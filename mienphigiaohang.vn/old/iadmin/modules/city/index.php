...
if($action=='add')
{
	...
	if($_POST && $_POST['title']!="")
	{
	 ...
	 $hide=$_POST['hide'];
	  ...
	  mysql_query("INSERT INTO city (title,hide) values('{$title}','{$hide}')") or die ('Khong the insert');
...
elseif($action=='edit')
{
	...
	if($_POST && $_POST['title']!=="")
	{
		 ...
		 mysql_query("UPDATE city SET title='{$title}', hide='{$hide}' where id='".$id."'") ;
		$mess=alert_success($config['domain'],"Cập nhật thành phố thành công!");
    	$smarty->assign('mess',$mess);
	...
	$listques=list_city(" where id='".$id."'");
...
elseif($_REQUEST['action']=='del')
{
	...
	mysql_query("DELETE FROM city where id='".$id."'");
...
else
{
	...
	if($_POST['submit'])
	{
		if($_POST['Selected'])
		{
			foreach($_POST['Selected'] as $udel)
			{
				...
				mysql_query("DELETE FROM city WHERE id='$udel'");
	...
	$url_page="do=city&id_module=".$id_module.$psearch;
	...
	$listques=list_city($where,$limit);
	$smarty->assign('info1',$listques);
