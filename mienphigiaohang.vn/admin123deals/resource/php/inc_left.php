...
$sql = '';
if($isAdmin != 1){
	$sql = ' INNER JOIN admin_user ON(adm_id = ' . $user_id . ')';
}

$query_order = "SELECT * FROM admin_menu_order WHERE amo_admin = " . $user_id . " ORDER BY amo_order ASC";
$db_order = new db_query($query_order);

$query_menu = "SELECT * FROM modules " . $sql . "ORDER BY mod_order ASC";

$db_menu = new db_query($query_menu);
...
<ul id="test-list">
	...
	foreach($arrayModule as $key=>$row){
		if(file_exists("modules/" . $row["mod_path"] . "/inc_security.php")===true){
			$i++;
            //Check quy?n truy c?p module! N?u k có quy?n thì b?!            
            if(checkaccessmodule($row["mod_id"])==0) continue;
			?>
			<li id="listItem_<?=$row["mod_id"]?>">
				<table cellpadding="3" cellspacing="0" width="100%">
					...
					<tbody id="showmneu_<?=$i?>" style="background-color: #FFFFFF; display:none;">
						<?
