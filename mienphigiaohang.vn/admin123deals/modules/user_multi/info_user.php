...
//Tìm theo keyword
if($keyword != ""){	
		$sqlWhere	.= " AND ( name LIKE '%" . $keyword . "%' OR username LIKE '%" . $keyword . "%' OR email LIKE '%" . $keyword . "%' OR tel LIKE '%" . $keyword . "%' OR date LIKE '%" . $keyword . "%') ";
	}
...
?>
	...
	<tr id="tr_<?=$No?>" <?=$fs_change_bg?>>
		<td class="No"><?=$No?></td>
		<td align="center" style="font-weight:bold"><?=$listing["username"]?></td>
		<td align="center"><?=$listing["name"]?></td>
		<td align="center"><?=$listing["email"]?></td>
		<td align="center"><?=$listing["tel"]?></td>
		<td align="center">
		...
		<td align="center"><a href="active.php?record_id=<?=$listing["id"]?>&redirect=<?=base64_encode(getURL())?>"><img border="0" src="<?=$fs_imagepath?>active_<?=$listing["hide"]?>.gif" /></a></td>
		
		<td align="center"><a title="Sửa dữ liệu" href="edit.php?record_id=<?=$listing["id"]?>&redirect=<?=base64_encode(getURL())?>"><img border="0" hspace="5" src="<?=$fs_imagepath?>edit.gif"></a></td>
		<td align="center"><img title="Xóa dữ liệu" hspace="5" src="<?=$fs_imagepath?>delete.gif" style="cursor:pointer" onClick="if(confirm('Bạn có muốn xóa tài khoản này không?')){window.location.href='delete.php?record_id=<?=$listing["id"]?>&redirect=<?=base64_encode(getURL())?>'}" /></td>
	</tr>
	<?=$form->hidden("record_id_" . $No, "record_id", $listing["id"], "");?>
