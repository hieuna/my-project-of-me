...
while($cat = mysql_fetch_array($db_category->result)){
	...
	$db_product = new db_query("SELECT pro_picture,pro_id,pro_name,pro_price,pro_teaser,pro_khuyenmai,pro_new,pro_category,pro_picture,pro_sp_khuyenmai
										 FROM products
			...
			?>
				...
				if($go_next == 1){
					$lin_pro = createLink("detail",array('module'=>$cat["cat_type"],"title"=>$row["pro_name"],"iCat"=>$row["pro_category"],"iData"=>$row["pro_id"]),$lang_path,$con_extenstion,$con_mod_rewrite);
					 $tooltip	= tooltip($row["pro_name"],"<img width='200' src='../pictures_products/medium_" . $row["pro_picture"] . "'>",(trim($row["pro_teaser"])!='') ? $row["pro_teaser"] : '',$row["pro_khuyenmai"],$con_currency);;
					 $strhotnew = '';
				...
				?>
					...
					<div style="background:url(/images/bg_pro.gif) repeat-y center; line-height:14px; padding:4px;">
					  <div style="height:40px; line-height:15px;">
                        	<a href="<?=$lin_pro?>" <? if($tooltip!=''){?> onmouseover="showtip('<?=$tooltip?>')" onmouseout="hidetip();" <? }?>><?=$row["pro_name"]?></a>
                            <? if($row["pro_price"] > 0){ ?><div style="color:#5e5e5e; z-index:10"><span style="color:#FF0000; padding-right:7px;"><?=formatNumber($row["pro_price"])?></span><?=$con_currency?></div> 
									<?  } else { ?>
								  <div style="color:#FF0000;"> Li&ecirc;n h&#7879; 043-7478341</div>
									  <? } ?>
					    </div>
						</div>
					</div>
