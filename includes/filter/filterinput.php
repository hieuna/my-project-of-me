...
/**
 * PGFilterInput is a class for filtering input from any data source
 *
 */
class PGFilterInput
{
	...
	function & getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
	{
		...
		if (empty ($instances[$sig])) {
			$instances[$sig] = new PGFilterInput($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
		}
	...
	function clean($source, $type='string')
	{
		...
		switch (strtoupper($type))
		{
			...
			case 'STRING' :
				...
				if(isset($this) && is_a( $this, 'PGFilterInput' )) {
					$filter =& $this;
				} else {
					$filter =& PGFilterInput::getInstance();
				}
			...
			default :
				// Check for static usage and assign $filter the proper variable
				if(is_object($this) && get_class($this) == 'PGFilterInput') {
					$filter =& $this;
				} else {
					$filter =& PGFilterInput::getInstance();
				}
	...
	function _cleanAttributes($attrSet)
	{
		...
		for ($i = 0; $i < count($attrSet); $i ++)
		{
			...
			if (PGFilterInput::checkAttribute($attrSubSet)) {
				continue;
			}
