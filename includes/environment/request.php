 ...
 */
$GLOBALS['_PGREQUEST'] = array();

/**
 * Set the available masks for cleaning variables
 */
define( 'PGREQUEST_NOTRIM'   , 1 );
define( 'PGREQUEST_ALLOWRAW' , 2 );
define( 'PGREQUEST_ALLOWHTML', 4 );
...
class PGRequest
{
	...
	function getVar($name, $default = null, $hash = 'default', $type = 'none', $mask = 0)
	{
		...
		if (isset($GLOBALS['_PGREQUEST'][$name]['SET.'.$hash]) && ($GLOBALS['_PGREQUEST'][$name]['SET.'.$hash] === true)) {
			// Get the variable from the input hash
			$var = (isset($input[$name]) && $input[$name] !== null) ? $input[$name] : $default;
			$var = PGRequest::_cleanVar($var, $mask, $type);
		}
		elseif (!isset($GLOBALS['_PGREQUEST'][$name][$sig]))
		{
			if (isset($input[$name]) && $input[$name] !== null) {
				// Get the variable from the input hash and clean it
				$var = PGRequest::_cleanVar($input[$name], $mask, $type);

				// Handle magic quotes compatability
				if (get_magic_quotes_gpc() && ($var != $default) && ($hash != 'FILES')) {
					$var = PGRequest::_stripSlashesRecursive( $var );
				}

				$GLOBALS['_PGREQUEST'][$name][$sig] = $var;
			}
			elseif ($default !== null) {
				// Clean the default value
				$var = PGRequest::_cleanVar($default, $mask, $type);
			}
			else {
				$var = $default;
			}
		} else {
			$var = $GLOBALS['_PGREQUEST'][$name][$sig];
		}
	...
	{
		return PGRequest::getVar($name, $default, $hash, 'int');
	}
	...
	{
		return PGRequest::getVar($name, $default, $hash, 'float');
	}
	...
	{
		return PGRequest::getVar($name, $default, $hash, 'bool');
	}
	...
	{
		return PGRequest::getVar($name, $default, $hash, 'word');
	}
	...
	{
		return PGRequest::getVar($name, $default, $hash, 'cmd');
	}
	...
	function getString($name, $default = '', $hash = 'default', $mask = 0)
	{
		...
		return (string) PGRequest::getVar($name, $default, $hash, 'string', $mask);
	}
	...
	function setVar($name, $value = null, $hash = 'method', $overwrite = true)
	{
		...
		$GLOBALS['_PGREQUEST'][$name]['SET.'.$hash] = true;
		$GLOBALS['_PGREQUEST'][$name]['SET.REQUEST'] = true;
	...
	function get($hash = 'default', $mask = 0)
	{
		...
		$result = PGRequest::_cleanVar($input, $mask);

		// Handle magic quotes compatability
		if (get_magic_quotes_gpc() && ($hash != 'FILES')) {
			$result = PGRequest::_stripSlashesRecursive( $result );
		}
	...
	function set( $array, $hash = 'default', $overwrite = true )
	{
		foreach ($array as $key => $value) {
			PGRequest::setVar($key, $value, $hash, $overwrite);
		}
	}
	...
	function clean()
	{
		PGRequest::_cleanArray( $_FILES );
		PGRequest::_cleanArray( $_ENV );
		PGRequest::_cleanArray( $_GET );
		PGRequest::_cleanArray( $_POST );
		PGRequest::_cleanArray( $_COOKIE );
		PGRequest::_cleanArray( $_SERVER );

		if (isset( $_SESSION )) {
			PGRequest::_cleanArray( $_SESSION );
		}
	...
	function _cleanVar($var, $mask = 0, $type=null)
	{
		...
		elseif ($mask & 4)
		{
			...
			if (is_null($safeHtmlFilter)) {
				$safeHtmlFilter = & PGFilterInput::getInstance(null, null, 1, 1);
			}
		...
		else
		{
			...
			if (is_null($noHtmlFilter)) {
				$noHtmlFilter = & PGFilterInput::getInstance(/* $tags, $attr, $tag_method, $attr_method, $xss_auto */);
			}
	...
	{
		$value = is_array( $value ) ? array_map( array( 'PGRequest', '_stripSlashesRecursive' ), $value ) : stripslashes( $value );
		return $value;
	}
...
/**
	...
	function getUserState( $key )
	{
		...
		$registry	=$session_object->get($key);
	...
	*/
	...
	{
		$session_object	=& PGSession::getInstance();
		$registry	=$session_object->get($key, null);

		return $session_object->set($key, $value);
	}
	 ...
	 */
	function getFilter( $key, $request, $default = null, $type = 'none' )
	{
		$old_state = PGRequest::getUserState( $key );
		$cur_state = (!is_null($old_state)) ? $old_state : $default;
		$new_state = PGRequest::getVar($request, null, 'default', $type);

		// Save the new value only if it was set in this request
		if ($new_state !== null) {
			PGRequest::setUserState($key, $new_state);
		} else {
			$new_state = $cur_state;
		}
