...
class PGURI
{
	...
	/**
	 * Returns a reference to a global PGURI object, only creating it
	 * if it doesn't already exist.
	 *
	 * This method must be invoked as:
	 * 		<pre>  $uri =& PGURI::getInstance([$uri]);</pre>
	 *
	 * @static
	 * @param	string $uri The URI to parse.  [optional: if null uses script URI]
	 * @return	PGURI  The URI object.
	 * @since	1.5
	 */
	function &getInstance($uri = 'SERVER')
	{
		...
		if (!isset ($instances[$uri]))
		{
			...
			// Create the new PGURI instance
			$instances[$uri] = new PGURI($theURI);
		}
	...
	function root($pathonly = false, $path = null)
	{
		...
		if(!isset($root))
		{
			$uri	        =& PGURI::getInstance(PGURI::base());
	...
	function isInternal($url) {
		$uri =& PGURI::getInstance($url);
		$base = $uri->toString(array('scheme', 'host', 'port', 'path'));
		$host = $uri->toString(array('scheme', 'host', 'port'));
		if(stripos($base, PGURI::base()) !== 0 && !empty($host)) {
			return false;
		}
		return true;
	}
	...
	function buildURL($url, $arr_param){
		if (!is_array($arr_param)) return false;
		
		/* Bước 1. Kiểm tra  biến $url xem có '?' không, nếu không có thì bổ sung vào*/
		if (strpos($url, '?') === false)
		{
			$url .= '?';
		}
		else if (substr($url, strlen($url)-1, 1) != '?' && substr($url, strlen($url)-1, 1) != '&')
		{
