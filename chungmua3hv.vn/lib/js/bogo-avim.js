/*
 *  AVIM JavaScript Vietnamese Input avim_method Source File dated 03-09-2007
 *
...
avim_va="email,username,password".split(',') //Put the ID of the fields you DON'T want to let users type Vietnamese in, multiple fields allowed, separated by a comma (,)
avim_method=0 //Default input avim_method, 0=AUTO, 1=TELEX, 2=VNI, 3=VIQR, 4=VIQR*
avim_on_off=1 //Start AVIM on
avim_dockspell=1 //Start AVIM with spell checking on
avim_dauCu=1 //Start AVIM with old way of marking accent (o`a, o`e, u`y)
avim_useCookie=1 //Set this to 0 to NOT use cookies
avim_radioID="him_auto,him_telex,him_vni,him_viqr,him_viqr2,him_off,him_ckspell,him_daucu".split(",")
...
function set_Avim_Turn(value){
	if(value=="On"){ avim_on_off= 1;}
	if(value=="Off"){ avim_on_off= 0;}
}
function set_Avim_Off(){
	avim_on_off= 0;
}
function set_Avim_Method(method){
	avim_on_off=1;
	
	if(method=="AUTO"){ avim_method= 0;}
	if(method=="TELEX"){ avim_method= 1;}
	if(method=="VNI"){ avim_method= 2;}
	if(method=="VIQR"){ avim_method= 3;}
	if(method=="VIQR*"){ avim_method= 4;}
}
//inputs_id= "id1, id2, ...";
function set_Avim_Not_Effect(string_id){
	avim_va= string_id.split(',');
}
...
function start(obj,key) {
	...
	if(avim_method==0) { uni="D,A,E,O,W,W".split(','); uni2="9,6,6,6,7,8".split(','); D2="DAWEO6789" }
	else if(avim_method==1) { uni="D,A,E,O,W,W".split(','); D2="DAWEO" }
	else if(avim_method==2) { uni="9,6,6,6,7,8".split(','); D2="6789" }
	else if(avim_method==3) { uni="D,^,^,^,+,(".split(','); D2="D^+(" }
	else if(avim_method==4) { uni="D,^,^,^,*,(".split(','); D2="D^*(" }
	if(!is_ie) {
		key=fcc(key.which)
		w=mozGetText(obj)
		...
		if(!avim_dockspell) w=mozGetText(obj)
...
function findC(w,k,sf) {
	if(((avim_method==3)||(avim_method==4))&&(w.substr(w.length-1,1)=="\\")) return new Array(1,k.charCodeAt(0))
	...
	if((c==1)||(uk==Z)) return vowA[0]
		...
		if((avim_dauCu==0)&&((ttt=="UY")||(ttt=="OA")||(ttt=="OE"))) return vowA[0]
...
function statusMessage() {
	var str='Kiểu gõ: '
	if(avim_on_off==0) str+='Tắt'
	else if(avim_method==1) str+='TELEX'
	else if(avim_method==2) str+='VNI'
	else if(avim_method==3) str+='VIQR'
	else if(avim_method==4) str+='VIQR*'
	else if(avim_method==0) str+='Tự động'
	if(isKHTML) str+=" [Alt-F9]"
	else str+=" [F9]"
	str+=" | Chính tả: "
	str+=(avim_dockspell==0)?"Tắt":"Bật"
	if(isKHTML) str+=" [Alt-F8]"
	else str+=" [F8]"
	str+=" | Bỏ dấu: "
	str+=(avim_dauCu==1)?"Cũ":"Mới"
	if(isKHTML) str+=" [Alt-F7]"
	else str+=" [F7]"
	str+=" | Bật/Tắt [F12] - AVIM 20070903"
	//window.status=str
}
function updateInfo() { setCookie(); if(support) statusMessage() }
function setMethod(m) {
	if(m==-1) { avim_on_off=0;if(getEL(avim_radioID[5])) getEL(avim_radioID[5]).checked=true }
	else { avim_on_off=1;avim_method=m;if(getEL(avim_radioID[m])) getEL(avim_radioID[m]).checked=true }
	setSpell(avim_dockspell);setDauCu(avim_dauCu);updateInfo()
}
function setDauCu(box) {
	if(typeof(box)=="number") {
		avim_dauCu=box;if(getEL(avim_radioID[7])) getEL(avim_radioID[7]).checked=box
	} else avim_dauCu=(box.checked)?1:0
	updateInfo()
}
function setSpell(box) {
	if(typeof(box)=="number") { 
		spellerr=(box==1)?ckspell:nospell
		if(getEL(avim_radioID[6])) getEL(avim_radioID[6]).checked=box
	}
	else {
		if(box.checked) { spellerr=ckspell;avim_dockspell=1 }
		else { spellerr=nospell;avim_dockspell=0 }
	}
	updateInfo()
}
function onKeyDown(e) {
	...
	if((key==120)||(key==123)||(key==119)||(key==118)) {
		if(key==120) { avim_on_off=1;setMethod(((avim_method==4)?0:++avim_method)) }
		else if(key==118) { setDauCu(((avim_dauCu==1)?0:1)) }
		else if(key==119) { avim_dockspell=(avim_dockspell==0)?1:0;setSpell(avim_dockspell) }
		else if(key==123) {
			avim_on_off=(avim_on_off==0)?1:0
			if(avim_on_off==0) setMethod(-1)
			else setMethod(avim_method)
		}
		updateInfo()
	}
}
...
function checkCode(code) { if(((avim_on_off==0)||((code<45)&&(code!=42)&&(code!=32)&&(code!=39)&&(code!=40)&&(code!=43))||(code==145)||(code==255))) return true; return false }
function fcc(x) { return String.fromCharCode(x) }
if(avim_useCookie==1) { setCookie=doSetCookie; getCookie=doGetCookie }
...
function doSetCookie() {
	...
	document.cookie='AVIM_on_off='+avim_on_off+end
	document.cookie='AVIM_method='+avim_method+end
	document.cookie='AVIM_ckspell='+avim_dockspell+end
	document.cookie='AVIM_daucu='+avim_dauCu+end
}
function doGetCookie() {
	...
	for(var i=0;i<ckA.length;i++) {
		...
		if(p[0]=='AVIM_on_off') avim_on_off=p[1]
		else if(p[0]=='AVIM_method') avim_method=p[1]
		else if(p[0]=='AVIM_ckspell') {
			if(p[1]==0) { avim_dockspell=0; spellerr=nospell }
			else { avim_dockspell=1; spellerr=ckspell }
		} else if(p[0]=='AVIM_daucu') avim_dauCu=parseInt(p[1])
	}
}
if(!is_ie) {
...
function findIgnore(el) {
	for(i=0;i<avim_va.length;i++) if((el.id==avim_va[i])&&(avim_va[i].length>0)) return true
}
if((is_ie)||(ver>=1.3)||(is_opera)||(isKHTML)) {
	getCookie()
	if(avim_on_off==0) setMethod(-1)
	else setMethod(avim_method)
	setSpell(avim_dockspell);setDauCu(avim_dauCu);statusMessage()
} else support=false
...
if((support)&&(!isKHTML)) {
	...
	for(g=0;g<fID.length;g++) {
		...
		if(is_ie) {
			...
			var iframedit
			try { wi=fID[g].contentWindow;iframedit=wi.document;iframedit.wi=wi } catch(e) { }
...
set_Avim_Turn("Off"); //set_Avim_Off();
set_Avim_Method("TELEX");
set_Avim_Not_Effect("txtUsername,txtPassword,txtEmail");