...
class ZoneBack extends VS_Module_Base  
{
	function __construct(){
		@eval(getGlobalVars());
		parent::__construct($oDb,$oSmarty);
		$this->table = "tbl_zones";
	 ...
	 //	$oDatagrid->setTable($table);		
		...
		$arr_cols= array(					
			...
			array(
				...
				"style" =>" font-size:11px; font-weight:bold; text-align:left; width:100px;",
				),
			array(
				"field" => "Z_Description",
				"display" => "Miêu tả",
				"style" =>" font-size:11px; text-align:left; width:200px;",
				)
		);		
		...
		$oDatagrid->addFilter(
			array(
				'field'=> array('name'=>array("Tên vùng quảng cáo",'text')),
				'type'=>'group',
				'name' => 'id_title',
		...
		if($_GET["id_title_group"]=='content'){
			...
			if($_GET["id_title"])
				$where.=" and Z_Name like '%".$_GET["id_title"]."%' ";
	...
	function buildForm ( $task, $arrData = array() )
	{
		...
		$form -> addElement('text', 'Z_Name',  "Tiêu đề", array('size' => 50, 'maxlength' => 255) );
		$form -> addElement('textarea', 'Z_Description',  "Miêu tả", array('cols' => 90, 'rows' => 5) );
		...
		$form->addElement( 'hidden', 'id', $arrData['Z_ID'] );
		$form -> addRule( 'title',$this->get_config_vars('title').$this->get_config_vars('no_blank'),'required');
		
		if( $form -> validate())
		{
			$aData = array(
				'Z_Name' => stripslashes($_POST['Z_Name']),
				'Z_Description' => stripslashes($_POST['Z_Description'])
			);
	...
	function changeStatus()
	{
		$id = $_GET['id'];
		$status = $_GET['active'];		
		$this -> vsDb -> updateWithPk ( $id, array ( 'active' => $status));		
