<?php
class CategoryBack extends baseGroup
{
	var $table;
	var $pk;
	var $_prefix;
	...
	function buildForm ( $task, $arrData = array())
	{
		...
		$url = "?mod=admin&amod={$_REQUEST['amod']}&atask={$_GET['atask']}&atype=deal&task=changeLang&categoryid={$idt}&ajax";
...
//		}		
	...
	function listItem()
	{
		...
		if($_GET[$this->_prefix."Name"])
			...
			$oDatagrid->addField(array("field"=> $this->_prefix."Name","link"	=> "{$submit_url}&task=edit","display"=>$this->get_config_vars('title'),"sortable"=>true,"style"=>"text-align:left;"));		
