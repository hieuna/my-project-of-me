...
<script type="text/javascript" src="'.SITE_URL.'lib/ckeditor/ckeditor.js"></script>
<script type="text/javascript" src="'.SITE_URL.'lib/ckeditor/_samples/sample.js"></script>
...
<link href="'.SITE_URL.'lib/css/customer.css" rel="stylesheet" type="text/css" />
...
if(!$_GET["ajax"])echo '
<script type="text/javascript" src="'.SITE_URL.'lib/css/admin.js"></script>
</body>
</html>';
		}
	}
	...
	function runEXT()
	{
		...
		if($usertype == 1)
		{	
			$modules=$oDb->getAll("select * from admin_menu where parent_id=0 and showed='1' order by z_index asc");
			if(is_array($modules))
			{
				foreach($modules as $key=>$module)
				{
					
					$sub = $oDb -> getAll("select * from admin_menu where parent_id = ".$module['id']." and showed='1' order by z_index "); 
			...
			$sql = "select * from admin_menu where parent_id=0 and showed='1' order by z_index asc";
			$allModule = $oDb -> getAll( $sql );
						
			$sql = "select t1.* from admin_menu t1 where showed='1' and id in( select t2.module_id from tbl_module_roll t2 join tbl_usertype_moduleroll t3 on(t3.module_roll_id = t2.id ) where t3.user_type_id= '{$usertype}') order by z_index";
			$subs = $oDb -> getAll( $sql );
