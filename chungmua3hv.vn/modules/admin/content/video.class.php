...
class imageVideo extends VS_Module_Base  
{
	...
	//	$comment =  editor ($this->_prefix.'Content', $arrData[$this->_prefix.'Content'], array ("width" => "100%", "height"	=> "600px","skin"=>'v2') );
		...
		$form -> addElement('text', $this->_prefix.'File',  "URL(Lấy từ youtube.com)", array('size' => 50, 'maxlength' => 255) );
	...
	//	$form -> addElement('date', $this->_prefix.'CreateDate',  "Ngày", array('size' => 50, 'maxlength' => 255) );
		...
		if( $form -> validate())
		{
			$aData = array(
				...
				$this->_prefix.'File' => stripslashes($_POST[$this->_prefix.'File']),
			...
			if($aData[$this->_prefix.'Status']=='1')
				$this->query("update {$this->table} set {$this->_prefix}Status='0' where {$this->_prefix}LangID='".$aData[$this->_prefix.'LangID']."'");
