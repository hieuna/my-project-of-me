...
class GKIS_eSport_Image {
	...
	*/
	function translateName($name, $mod_id, $thumb = '') {
		$name = GKIS_eSport_Image::getRealPath($name);
	...
	*/
	...
	function createThumbnail($path, $config, $width, $height, $image_bg, $image_stretch, $quality, $thumb = '') {
		if(GKIS_eSport_Image::checkCache(GKIS_eSport_Image::translateName($path,$config['module_id'],$thumb), $config['last_modification'])){
			return TRUE;	
		}else{
			// importing classes
			...
			$cache_dir = JPATH_ROOT.DS.'modules'.DS.'mod_image_show_gk4'.DS.'cache'.DS;
			// file path
			$file = GKIS_eSport_Image::getRealPath($path);
			// filename
			$filename = GKIS_eSport_Image::translateName($path,$config['module_id']);
