...
class GKIS_gk_coffe_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function checkImages() {
		// basic images params		
		$img_width = $this->config['config']->gk_coffe->gk_coffe_image_width;
		$img_height = $this->config['config']->gk_coffe->gk_coffe_image_height;
		$img_bg = $this->config['config']->gk_coffe->gk_coffe_image_bg;
		$quality = $this->config['config']->gk_coffe->gk_coffe_quality;
		// check the slides
	...
	function getArticleData() {
		...
		// get the data
		if(count($idsK2) > 0) {
			$this->articlesK2 = GKIS_gk_coffe_Model::getDataK2($idsK2);
		}
		if(count($ids) > 0) {
			$this->articles = GKIS_gk_coffe_Model::getData($ids);
		}
	}
	// generate view
	function generateView() {
		...
		// add script fragment
		$document->addScriptDeclaration('try {$Gavick;}catch(e){$Gavick = {};};$Gavick["gkIs-'.$this->config['module_id'].'"] = { "anim_speed": '.$this->config['config']->gk_coffe->gk_coffe_animation_speed.', "anim_interval": '.$this->config['config']->gk_coffe->gk_coffe_animation_interval.', "autoanim": '.$this->config['config']->gk_coffe->gk_coffe_autoanimation.', "anim_type": "'.$this->config['config']->gk_coffe->gk_coffe_animation_type.'", "slide_links": '.$this->config['config']->gk_coffe->gk_coffe_slide_links.' };');
