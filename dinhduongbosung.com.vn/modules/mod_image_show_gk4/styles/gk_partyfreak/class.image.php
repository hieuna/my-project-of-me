...
class GKIS_PartyFreak_Image {
	...
	*/
	function translateName($name, $mod_id, $thumb = '') {
		$name = GKIS_PartyFreak_Image::getRealPath($name);
	...
	*/
	...
	function createThumbnail($path, $config, $width, $height, $image_bg, $image_stretch, $quality, $thumb = '') {
		if(GKIS_PartyFreak_Image::checkCache(GKIS_PartyFreak_Image::translateName($path,$config['module_id'],$thumb), $config['last_modification'])){
			return TRUE;	
		}else{
			// importing classes
			...
			$cache_dir = JPATH_ROOT.DS.'modules'.DS.'mod_image_show_gk4'.DS.'cache'.DS;
			// file path
			$file = GKIS_PartyFreak_Image::getRealPath($path);
			// filename
			$filename = GKIS_PartyFreak_Image::translateName($path,$config['module_id']);
			...
			if(is_file($file)){
				...
				if($imageData['mime'] == 'image/jpeg' || $imageData['mime'] == 'image/pjpeg' || $imageData['mime'] == 'image/jpg') imagejpeg($imageBG,$cache_dir.$thumb.$filename, $quality);
				elseif($imageData['mime'] == 'image/gif') imagegif($imageBG, $cache_dir.$thumb.$filename); 
				else imagepng($imageBG, $cache_dir.$thumb.$filename);
				return TRUE;
			}else{
				return FALSE;
			}	
