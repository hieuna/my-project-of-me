...
class GKIS_FinancialBusiness_Image {
	/*
	...
	*/
	function translateName($name, $mod_id) {
		$name = GKIS_FinancialBusiness_Image::getRealPath($name);
	...
	*/
	...
	function createThumbnail($path, $config, $width, $height, $image_bg, $image_stretch, $quality) {
		if(GKIS_FinancialBusiness_Image::checkCache(GKIS_FinancialBusiness_Image::translateName($path,$config['module_id']), $config['last_modification'], $config['module_id'])){
			return TRUE;	
		}else{
			// importing classes
			...
			$cache_dir = JPATH_ROOT.DS.'modules'.DS.'mod_image_show_gk4'.DS.'cache'.DS;
			// file path
			$file = GKIS_FinancialBusiness_Image::getRealPath($path);
			// filename
			$filename = GKIS_FinancialBusiness_Image::translateName($path,$config['module_id']);
