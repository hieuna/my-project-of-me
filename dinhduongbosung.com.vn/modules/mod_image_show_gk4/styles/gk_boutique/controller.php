...
class GKIS_gk_boutique_Controller {
	// configuration array
	private $config;
	// module info
	private $module;
	...
	function checkImages() {
		// basic images params		
		$img_width = $this->config['config']->gk_boutique->gk_boutique_image_width;
		$img_height = $this->config['config']->gk_boutique->gk_boutique_image_height;
		$img_bg = $this->config['config']->gk_boutique->gk_boutique_image_bg;
		$quality = $this->config['config']->gk_boutique->gk_boutique_quality;
		// check the slides
		foreach($this->config['image_show_data'] as $slide) {
			$stretch = ($slide->stretch == 'nostretch') ? false : true;
			GKIS_Boutique_Image::createThumbnail($slide->image, $this->config, $img_width, $img_height, $img_bg, $stretch, $quality);	
		}
	}
	// get the articles data
	function getArticleData() {
		...
		// get the data
		if(count($ids) > 0) {
			$this->articles = GKIS_gk_boutique_Model::getData($ids);
		}
		if(count($idsK2) > 0) {
			$this->articlesK2 = GKIS_gk_boutique_Model::getDataK2($idsK2);
		}
	}
	// generate view
	function generateView() {
		...
		// add script fragment
		$document->addScriptDeclaration('try {$Gavick;}catch(e){$Gavick = {};};$Gavick["gkIs-'.$this->config['module_id'].'"] = { "anim_speed": '.$this->config['config']->gk_boutique->gk_boutique_animation_speed.', "anim_interval": '.$this->config['config']->gk_boutique->gk_boutique_animation_interval.', "autoanim": '.$this->config['config']->gk_boutique->gk_boutique_autoanimation.', "anim_type": "'.$this->config['config']->gk_boutique->gk_boutique_animation_type.'","slide_links": '.$this->config['config']->gk_boutique->gk_boutique_slide_links.' };');
		// generate necessary variables
		$width = $this->config['config']->gk_boutique->gk_boutique_image_width;
		$height = $this->config['config']->gk_boutique->gk_boutique_image_height;
		// load view
